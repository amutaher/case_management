...
class GeneralClosureForm(Document):
	...
	def on_submit(self):
		...
		frappe.db.sql("UPDATE `tabGeneral Assessment` set docstatus=1 , status='Closed' where case_id=%s", (self.case_id))
		frappe.db.sql("UPDATE `tabGeneral Case` set docstatus=1 where , status='Closed' case_id=%s", (self.case_id))
