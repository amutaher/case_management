...
class CPClosureForm(Document):
	...
	def on_submit(self):
		...
		frappe.db.sql("UPDATE `tabCP Assessment` set docstatus=1 , status='Closed' where case_id=%s", (self.case_id))
		frappe.db.sql("UPDATE `tabCP Case` set docstatus=1 , status='Closed' where case_id=%s", (self.case_id))
